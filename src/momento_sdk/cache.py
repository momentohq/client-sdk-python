...
class Cache:
    ...
    def set(self, key , value, ttl_seconds=None):
        item_ttl_seconds = self._default_ttlSeconds if ttl_seconds == None else ttl_seconds
        set_request = cache_client_types.SetRequest()
        set_request.cache_key = self._asBytes(key, "Unsupported type for key: ")
        set_request.cache_body = self._asBytes(value, "Unsupported type for value: ")
        set_request.ttl_milliseconds = item_ttl_seconds * 1000
        self._client.Set(set_request)

    def get(self, key):
        get_request =  cache_client_types.GetRequest()
        get_request.cache_key = self._asBytes(key, "Unsupported type for key: ")
