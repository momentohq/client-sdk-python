...
class FixedTimeoutRetryStrategy(RetryStrategy):
    def __init__(
        self, *, retry_timeout_millis: int, retry_delay_interval_millis: int,
    ...
    def determine_when_to_retry(self, props: RetryableProps) -> Optional[float]:
        ...
        if (
            ...
            props.overall_deadline > datetime.now()
        ):
    ...
    def get_jitter_in_millis(self, props: RetryableProps) -> float:
        timeout_with_jitter = self.add_jitter(
            self._retry_delay_interval_millis
        )
        logger.debug("Determined request is retryable; retrying after %d ms: [method: %s, status: %s, attempt: %d]",
            timeout_with_jitter,
            props.grpc_method,
            props.grpc_status,
            props.attempt_number
